...
/**
 * The TextAML3TaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * TextAML3TaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class TextAML3TaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.emftext.language.textAML3.resource.textAML3.mopp.TextAML3TaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.emftext.language.textAML3.resource.textAML3.mopp.TextAML3TaskItem> foundItems = new java.util.ArrayList<org.emftext.language.textAML3.resource.textAML3.mopp.TextAML3TaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.emftext.language.textAML3.resource.textAML3.mopp.TextAML3TaskItem(keyword, message, localLine, offset, end));
