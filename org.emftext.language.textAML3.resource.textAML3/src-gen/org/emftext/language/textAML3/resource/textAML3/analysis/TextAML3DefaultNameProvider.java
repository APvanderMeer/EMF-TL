...
public class TextAML3DefaultNameProvider implements org.emftext.language.textAML3.resource.textAML3.ITextAML3NameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.emftext.language.textAML3.resource.textAML3.util.TextAML3EObjectUtil.invokeOperation(element, operation);
					if (result != null) {
